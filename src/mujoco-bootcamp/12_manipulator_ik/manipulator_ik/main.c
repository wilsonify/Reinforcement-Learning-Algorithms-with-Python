...
//simulation end time
double simend = 2*3.14/omega + 0.25;
...
void init_save_data()
{
  //write name of the variable here (header)
   fprintf(fid,"t, ");
   fprintf(fid,"xref,zref, ");
   fprintf(fid,"xact,zact ");
...
void save_data(const mjModel* m, mjData* d)
{
  ...
  fprintf(fid,"%f, %f, ", xref,zref);
  fprintf(fid,"%f, %f ", xact, zact);
...
// {
//   d->qpos[0] = Xin[0]; d->qpos[1] = Xin[1];
//   d->ctrl[0] = d->qpos[0]; d->ctrl[2] = d->qpos[1];
//   mj_forward(m,d);
//   Xout[0] = d->sensordata[0];
//   Xout[1] = d->sensordata[2];
...
void init_controller(const mjModel* m, mjData* d)
{
  ...
  //double Xref[2] = {1.75,1.5}; //reference position
  double Xout[2]={0};
  simulator(Xin,Xout);
  printf("%f %f \n",Xout[0],Xout[1]);
  // Xout[0] = center_x + a;
...
int main(int argc, const char** argv)
{
    ...
    double arr_view[] = {89.608063, -11.588379, 5, 0.000000, 0.000000, 1.000000}; //view the left side (for ll, lh, left_side)
