...
def clipped_surrogate_obj(new_p, old_p, adv, eps):
    ...
    return -tf.reduce_mean(
        tf.minimum(rt * adv, tf.clip_by_value(rt, 1 - eps, 1 + eps) * adv)
    )
...
def gaussian_log_likelihood(x, mean, log_std):
    ...
    """
    log_p = -0.5 * (
        (x - mean) ** 2 / (tf.exp(log_std) ** 2 + 1e-9)
        + 2 * log_std
        + np.log(2 * np.pi)
    )
...
def PPO(
    ...
    if action_type == "Discrete":
        with tf.variable_scope("actor_nn"):
            p_logits = mlp(
                obs_ph, hidden_sizes, act_dim, tf.nn.relu, last_activation=tf.tanh
            )
    ...
    # Computational graph for the policy in case of a continuous action space
        ...
        with tf.variable_scope("actor_nn"):
            p_logits = mlp(
                obs_ph, hidden_sizes, act_dim, tf.tanh, last_activation=tf.tanh
            )
        ...
        p_noisy = p_logits + tf.random_normal(tf.shape(p_logits), 0, 1) * tf.exp(
            log_std
        )
    ...
    if action_type == "Box":
        tf.summary.scalar(
            "p_std", tf.reduce_mean(tf.exp(log_std)), collections=["train"]
        )
    ...
    file_writer = tf.summary.FileWriter(
        "log_dir/" + env_name + "/PPO_" + clock_time + "_" + hyp_str,
    ...
    for ep in range(num_epochs):
        ...
        for env in envs:
            ...
            for _ in range(steps_per_env):
                ...
                # run the policy
                act, val = sess.run(
                    [act_smp, s_values], feed_dict={obs_ph: [env.n_obs]}
                )
                act = np.squeeze(act)
        ...
        for _ in range(actor_iter):
            ...
            for idx in range(0, lb, minibatch_size):
                ...
                sess.run(
                    p_opt,
                    feed_dict={
                        obs_ph: obs_batch[minib],
                        act_ph: act_batch[minib],
                        adv_ph: adv_batch[minib],
