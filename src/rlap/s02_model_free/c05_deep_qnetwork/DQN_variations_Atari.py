...
def DQN_with_variations(
    ...
    hyp_str = "-lr_{}-upTN_{}-upF_{}-frms_{}-ddqn_{}-duel_{}-nstep_{}".format(
        ...
        frames_num,
        extensions_hyp["DDQN"],
        extensions_hyp["dueling"],
        extensions_hyp["multi_step"],
    )
    ...
    buffer = MultiStepExperienceBuffer(
        buffer_size, extensions_hyp["multi_step"], discount
    )
        ...
        while not done:
            ...
            if len(buffer) > min_buffer_size and (step_count % update_freq == 0):
                ...
                if extensions_hyp["DDQN"]:
                    ...
                    y_r = double_q_target_values(
                        mb_rew, mb_done, mb_trg_qv, mb_onl_qv, discount
                    )
                else:
