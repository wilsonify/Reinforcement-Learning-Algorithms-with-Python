...
def gaussian_log_likelihood(ac, mean, log_std):
    ...
    """
    log_p = ((ac - mean) ** 2 / (tf.exp(log_std) ** 2 + 1e-9) + 2 * log_std) + np.log(
        2 * np.pi
    )
    ...
    return tf.reduce_mean(
        tf.reduce_sum(
            ...
            * (
                log_std_p
                - log_std_q
                + tf.exp(log_std_q - log_std_p)
                + (mu_q - mu_p) ** 2 / tf.exp(log_std_p)
                - 1
            ),
            axis=1,
        )
...
class Buffer:
    ...
    def store(self, temp_traj, last_sv):
        ...
        if len(temp_traj) > 0:
            ...
            self.adv.extend(
                GAE(temp_traj[:, 1], temp_traj[:, 3], last_sv, self.gamma, self.lam)
            )
            self.rtg.extend(rtg)
    ...
    def get_batch(self):
        ...
        return (
            np.array(self.ob),
            np.array(np.expand_dims(self.ac, -1)),
            np.array(norm_adv),
            np.array(self.rtg),
        )
...
class FullBuffer:
    ...
    def get_training_batch(self):
        return (
            np.array(self.obs)[self.train_idx],
            np.array(np.expand_dims(self.act, -1))[self.train_idx],
            np.array(self.rew)[self.train_idx],
            np.array(self.nxt_obs)[self.train_idx],
            np.array(self.done)[self.train_idx],
        )

    def get_valid_batch(self):
        return (
            np.array(self.obs)[self.valid_idx],
            np.array(np.expand_dims(self.act, -1))[self.valid_idx],
            np.array(self.rew)[self.valid_idx],
            np.array(self.nxt_obs)[self.valid_idx],
            np.array(self.done)[self.valid_idx],
        )
...
class NetworkEnv(gym.Wrapper):
    ...
    def step(self, action):
        ...
        obs = self.model_func(
            self.obs, [np.squeeze(action)], np.random.randint(0, self.number_models)
        )
...
def pendulum_done(ob):
    return np.abs(np.arcsin(np.squeeze(ob[3]))) > 0.2
...
def restore_model(old_model_variables, m_variables):
    ...
    for m_v in m_variables:
        upd_m_rsh = tf.reshape(
            old_model_variables[it_v2 : it_v2 + tf.reduce_prod(m_v.shape)],
            shape=m_v.shape,
        )
...
def METRPO(
    ...
    old_model_variables = tf.placeholder(
