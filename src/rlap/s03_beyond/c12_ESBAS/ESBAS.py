...
class DQN_optimization:
    ...
    def __build_graph(self):
        ...
        with self.g.as_default():
            ...
            self.obs_ph = tf.placeholder(
                shape=(None, self.obs_dim[0]), dtype=tf.float32, name="obs"
            )
    ...
    def optimize(self, mb_obs, mb_rew, mb_act, mb_obs2, mb_done):
        ...
        self.sess.run(
            self.v_opt,
            feed_dict={self.obs_ph: mb_obs, self.y_ph: y_r, self.act_ph: mb_act},
        )
...
class UCB1:
    ...
    def choose_algorithm(self):
        ...
        current_best = np.argmax(
            [
                self.xk[i] + np.sqrt(self.epsilon * np.log(self.n) / self.nk[i])
    ...
    def update(self, idx_algo, traj_return):
        ...
        self.xk[idx_algo] = (self.nk[idx_algo] * self.xk[idx_algo] + traj_return) / (
            self.nk[idx_algo] + 1
        )
...
def ESBAS(
    ...
    file_writer = tf.summary.FileWriter(
        LOG_DIR + "/ESBAS_" + clock_time + "_" + hyp_str, tf.get_default_graph()
    )
