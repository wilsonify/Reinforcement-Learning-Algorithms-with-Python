...
def worker(
    ...
    with tf.device("/cpu:" + worker_name):
        ...
        new_weights_ph = tf.placeholder(
            shape=(None,), dtype=tf.float32, name="new_weights_ph"
        )
        ...
        for a_v in agent_variables:
            upd_rsh = tf.reshape(
                new_weights_ph[it_v1 : it_v1 + tf.reduce_prod(a_v.shape)],
                shape=a_v.shape,
            )
        ...
        vars_grads_list = []
        for a_v in agent_variables:
            vars_grads_list.append(
                tf.reshape(
                    new_weights_ph[it_v2 : it_v2 + tf.reduce_prod(a_v.shape)],
                    shape=a_v.shape,
                )
            )
        ...
        apply_g = opt.apply_gradients(
            [(g, v) for g, v in zip(vars_grads_list, agent_variables)]
        )
    ...
    def evaluation_on_noise(noise):
        ...
        sess.run(
            update_weights,
            feed_dict={new_weights_ph: original_weights + noise * std_noise},
        )
...
def ES(
    ...
    file_writer = tf.summary.FileWriter(
        "log_dir/" + env_name + "/" + clock_time + "_" + hyp_str, tf.get_default_graph()
    )
