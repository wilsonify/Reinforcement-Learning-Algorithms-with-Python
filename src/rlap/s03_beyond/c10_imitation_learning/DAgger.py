...
def flappy_to_list(fd):
    ...
    return (
        fd["player_y"],
        fd["player_vel"],
        fd["next_pipe_dist_to_player"],
        fd["next_pipe_top_y"],
        fd["next_pipe_bottom_y"],
        fd["next_next_pipe_dist_to_player"],
        fd["next_next_pipe_top_y"],
...
def DAgger(
    ...
    p_loss = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits_v2(labels=act_onehot, logits=p_logits)
    )
    # Adam optimizer
