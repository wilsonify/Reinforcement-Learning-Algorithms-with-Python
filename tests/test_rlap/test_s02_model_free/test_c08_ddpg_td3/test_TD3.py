...
def deterministic_actor_double_critic(x, a, hidden_sizes, act_dim, max_act=1):
    ...
    with tf.variable_scope("q1_mlp"):
        q1_d = mlp(
            tf.concat([x, p_means], axis=-1), hidden_sizes, 1, last_activation=None
        )

    with tf.variable_scope(
        "q1_mlp", reuse=True
    ):  # Use the weights of the mlp just defined
...
def TD3(
    ...
    # Optimize the actor
    p_opt = tf.train.AdamOptimizer(ac_lr).minimize(
